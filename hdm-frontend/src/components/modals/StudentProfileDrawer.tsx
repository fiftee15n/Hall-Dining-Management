...
import { Phone, CheckCircle2, Pencil, Building, BookOpen } from 'lucide-react';
...
export function StudentProfileDrawer({ student, isOpen, onClose, onEdit }: StudentProfileDrawerProps) {
  ...
  return (
    <>
        ...
        <div className="space-y-5 text-sm">
          ...
          <div className="p-5 bg-slate-900 text-white rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-md">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              {/* Financial Balance Chips: Visible ONLY for Management Team & Admin */}
              {!isAuthority && (
                <>
              ...
              {onEdit && (
                <button
                  ...
                  className="px-3.5 py-2 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-xl text-xs transition border border-white/20 flex items-center gap-1.5"
              ...
              {canSettleDues && student.balanceDue > 0 && (
                <button
          ...
          {/* Authority View: Clean Resident Profile Info Card (No booking/payment/refund history) */}
          {isAuthority ? (
            <div className="space-y-4">
              <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-5 space-y-3.5">
                <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                  <Building className="w-4 h-4 text-slate-700" />
                  <span>Resident Hall Registration Details</span>
                </h4>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="p-3 bg-white rounded-xl border border-slate-200">
                    <span className="text-slate-400 font-bold block text-[10px] uppercase">Allotted Room</span>
                  ...
                  <div className="p-3 bg-white rounded-xl border border-slate-200">
                    <span className="text-slate-400 font-bold block text-[10px] uppercase">Department & Batch</span>
                    <span className="font-bold text-slate-900 text-sm mt-0.5 block">
                      {student.department} ({student.batch}th Batch)
                    </span>
                  </div>

                  <div className="p-3 bg-white rounded-xl border border-slate-200">
                    <span className="text-slate-400 font-bold block text-[10px] uppercase">Student ID / Roll</span>
                    <span className="font-bold text-slate-900 text-sm mt-0.5 block">
                      {student.studentId}
                    </span>
                  </div>
                  ...
                  <div className="p-3 bg-white rounded-xl border border-slate-200">
                    <span className="text-slate-400 font-bold block text-[10px] uppercase">Residency Status</span>
                    <span className="font-bold text-emerald-800 text-sm mt-0.5 block">
                      {student.status === 'inactive' ? 'On Leave' : 'Active Resident'}
                    </span>
                  </div>
                </div>
              </div>
            ...
            /* Management Team & Admin View: Complete Order, Refund, and Payment History */
            <>
              {/* Bookings Section */}
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {studentBookings.map((b) => (
                          <tr key={b.id} className="hover:bg-slate-50/70">
                            ...
                            <td className="p-3 text-slate-600">{b.totalLunchCount} Lunch + {b.totalDinnerCount} Dinner</td>
      ...
      {/* Record payment modal rendered only for Management Team / Admin */}
      {!isAuthority && (
        <RecordPaymentModal
          isOpen={showPayModal}
